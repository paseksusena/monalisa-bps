<form  action="/administrasi/file/addFile?transaksi={{ $transaksi->id }}&akun={{ $akun->id }}&kegiatan={{ $kegiatan->id }}&periode={{ $periode->slug }}&fungsi={{ $fungsi }}"
    method="post" enctype="multipart/form-data" class="flex items-center justify-between p-4 bg-white border rounded-lg border-gray-300">
    @csrf
    <!-- Input File Multiple -->
    ...
    <input
        ...
        id="formFile" type="file"  for="formFile" name="files[]" multiple>
    <!-- Tombol Submit -->
    <button type="submit"
